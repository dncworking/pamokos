...
function findItemById(id) {
  ...
  const found = items.find((items) => items.id === 1);
  return found;
}
...
function capitalizeNames() {
  ...
  return items.map((item) => ({
    ...
    name: item.name.charAt(0).toUpperCase() + item.name.slice(1).toLowerCase(), // Capitalize the name
  }));
}
...
function calculateTotalInventory() {
  ...
  const result = items.reduce(
    (accumulator, items) => accumulator + items.inventory,
    0
  );
  return result;
}
...
function getItemPriceByName(name) {
  ...
  const result = items.find((fruit) => fruit.name === "apple");
  return result.price;
}
